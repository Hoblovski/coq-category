...
Class Functor
      {Ob1 Ob2: Type} (C : Category Ob1) (D : Category Ob2) :=
  {
    fobj      : Ob1 -> Ob2;
...
Implicit Arguments fobj [ Ob1 Ob2 C D ].
Implicit Arguments fmor [ Ob1 Ob2 C D a b ].
...
Lemma Functor_eq :
  forall `(C: Category) `(D: Category) (F G: Functor C D)
    ...
    ((fun a b =>
        ...
        with
          eq_refl => @fmor _ _ _ _ F a b
        end)
     = (fmor G))
    -> F = G.
Proof.
  intros ObC C ObD D F G Oeq Meq.
...
Example fnt_const :
  forall ObC ObD (C: Category ObC) (D: Category ObD) (b: ObD),
    Functor C D.
Proof.
  intros ObC C ObD D b.
  apply Build_Functor
...
Defined.
Notation "G <<< F" := (@fnt_comp _ _ _ _ _ _ F G)  :category_scope.

Lemma fnt_id_l :
  forall `(F: Functor), F <<< (fnt_id C) = F.
Proof.
  intros Ob1 Ob2 C D F.
...
Lemma fnt_assoc :
  forall `{C0: Category} `{C1: Category} `{C2: Category} `{C3: Category}
    ...
    H <<< (G <<< F) = (H <<< G) <<< F.
Proof.
  intros O0 C0 O1 C1 O2 C2 O3 C3 F G H.
...
Class F_Isomorphic `(C: Category) `(D: Category) : Prop :=
  {
...
Lemma finv_symm :
  forall `(F: Functor C D) G,
    F_Inversion F G -> F_Inversion G F.
Proof.
  intros Ob1 Ob2 C D F G HFG.
...
Lemma fnt_equal :
  forall `{C: Category} `{D: Category}
    (F: Functor C D) (G H: Functor D C),
    G <<< F = fnt_id C -> F <<< H = fnt_id D -> G = H.
Proof.
  intros Ob1 C Ob2 D F G H Hfg Hhf.
  rewrite <- fnt_id_r.
  rewrite <- Hfg.
  rewrite <- fnt_assoc.
  rewrite -> Hhf.
...
Theorem double_finv :
  forall `(F: Functor C D) G H,
    F_Inversion F G -> F_Inversion G H -> F = H.
Proof.
  intros Ob1 Ob2 C D F G H Hfg Hgh.
...
Theorem finv_comp :
  forall `{C: Category} `{D: Category} `{E: Category}
    (F: Functor C D) (G: Functor D E) F' G',
    F_Inversion F F' ->
    F_Inversion G G' ->
    F_Inversion (G <<< F) (F' <<< G').
Proof.
  intros Ob1 C Ob2 D Ob3 E F G F' G' HF HG.
...
Proof.
  intros Ob1 C Ob2 D Ob3 E F G HF HG.
  apply Build_F_Isomorphism.
